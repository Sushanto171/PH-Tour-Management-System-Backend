...
const sslPaymentInit = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = {
            ...
            success_url: `${env_1.envVars.SSL.SSL_BACKEND_SUCCESS_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=success`,
            cancel_url: `${env_1.envVars.SSL.SSL_BACKEND_CANCEL_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=canceled`,
            fail_url: `${env_1.envVars.SSL.SSL_BACKEND_FAIL_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=failed`,
...
const validatePayment = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield payment_model_1.Payment.findOneAndUpdate({ transactionId: response.data.tran_id }, { paymentGatewayData: response.data }, {
            runValidators: true,
        });
    }
    catch (error) {
        console.log("Payment validation error:", error.message);
        throw new AppError_1.AppError(500, "Payment validation error");
    }
});
